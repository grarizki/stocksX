...
import { ArrowLeft, TrendingDown, TrendingUp } from "lucide-vue-next";
...
const indicator = computed(
	() => indicators.find((i) => i.id === id.value) ?? null,
);

useHead({
	title: computed(() =>
		indicator.value ? `${indicator.value.name} - StoxLyz` : "Indicator",
	),
});
		...
		datasets: [
			{
				...
				backgroundColor: (ctx: any) => {
					...
					gradient.addColorStop(
						0,
						isPositive.value ? "rgba(34,197,94,0.18)" : "rgba(239,68,68,0.18)",
					);
